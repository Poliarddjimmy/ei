...
var weeklySalesInit = function weeklySalesInit() {
  ...
  // create my data from php data
  var dataArray = [];
  var days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

  days.forEach(function (day) {
    var data = requestspaidThisWeekReport.find(item => item.name === day);
    if (data) {
      dataArray.push({ name: day, count: data.count });
    } else {
      dataArray.push({ name: day, count: 0 });
    }
  });
  ...
  if ($echartBarWeeklySales) {
    ...
    var data = dataArray.map(item => item.count);
